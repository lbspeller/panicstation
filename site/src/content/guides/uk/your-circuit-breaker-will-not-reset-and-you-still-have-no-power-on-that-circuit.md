...
tags:
  ...
  - sockets not working one circuit
  - lights out on one circuit
  - breaker trips immediately
  - breaker clicks off again
  - electrics tripping repeatedly
  - possible electrical fault
  - burning smell from fuse box
  - damp socket tripping
...
## What to do now
1. **Pause and check for danger signs.** If you smell burning, see smoke, hear crackling/arcing, or the consumer unit is hot: **switch off the main switch only if it’s safe to do so**, move people away, and call **999** for the fire service if there’s any sign of fire/smoke.
2. **Make sure it really is just one circuit.** If multiple circuits (or the whole home) are off, or neighbours are also out, treat it as a **power cut** rather than a single-circuit fault and contact your electricity network operator (in Great Britain you can call **105** for power-cut help).
3. **Unplug everything on the affected circuit.** Unplug appliances and chargers in the rooms that are out. Also switch off anything hard-wired you can safely isolate (e.g., fused spur to an appliance) without opening covers.
4. **Reset safely (one attempt, then stop).**
...
5. **If it won’t reset or trips immediately, keep it OFF.** That usually means a continuing fault (in wiring, a socket, a light fitting, water ingress, or an appliance).
6. **Do a quick “what changed?” check (no dismantling).** Think about anything recent on that circuit: a heater, kettle, tumble dryer, iron; a new appliance; DIY drilling; a wet area (bathroom/kitchen/outdoor socket); a bulb change; flooding/condensation. This helps you describe the problem clearly.
7. **Contact the right person for your housing situation.**
...
8. **Keep the rest of the home safe while you wait.**
